...
const RethinkDB = {
  ...
  install(Vue, options) {
    ...
    RethinkDB.rws.addEventListener('message', message => {
      ...
      try {
        ...
        console.log('RethinkDB: Error parsing message from server', e);
        return;
      }

      // Silently handle Primus heartbeats
    ...
    setInterval(() => {
      const now = Date.now();
      Object.keys(RethinkDB.requests).forEach(rid => {
        const request = RethinkDB.requests[rid];
        if (now > request.timestamp + 3000) {
          RethinkDB.options.log.error('Expired request', request.rid);
          delete RethinkDB.requests[rid];
  ...
  registerField(Vue, vm, field, config) {
    ...
    const query = {
      ...
      type: config.value ? 'value' : 'collection',
    ...
    const element = config.type === 'value' ? {} : [];
    if (field in vm) {
      vm[field] = element;
    } else {
  ...
  subscribe(query) {
    ...
    if (RethinkDB.rws.readyState === WebSocket.OPEN) {
      RethinkDB.emit('subscribe', { queryId: query.queryId, query: query.query, params: query.params });
    }
  },
  ...
  emitAck(event, data) {
    ...
    const request = {};
    const rid = RethinkDB.nextRequestId++;

    const promise = new Promise((resolve, reject) => {
      request.resolve = resolve;
      request.reject = reject;
      request.timestamp = Date.now();
      RethinkDB.requests[rid] = request;
    });
